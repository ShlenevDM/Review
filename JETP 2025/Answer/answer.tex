...
\begin{document}
\begin{center}
Ответ на отзыв рецензента на работу\\
...
В отзыве рецензента было указано, что необходимо прояснение математических 
условий справедливости формулы (21).

В ответ на замечание рецензента нами были внесены в статью следующие изменения:
\begin{enumerate}
	\item Уточнена формулировка условия выполнимости формулы (21). 
	\item В формуле (21) был указан порядок слагаемых, который были отброшены в 
	дельта-функциональном приближении.
	\item Добавлено приложение 1, в котором приведено более строгое 
	математическое обоснование допустимости применения такой аппроксимации (21).
	\item Отмечено, что равенство нулю реальной части физически соответствует 
	тому, что электрон выходит на массовую поверхность и становится реальным.
\end{enumerate} 
