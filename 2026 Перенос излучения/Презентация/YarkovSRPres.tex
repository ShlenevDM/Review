	...
	\begin{columns}
		...
		\begin{column}{0.60\linewidth}
			\begin{center}
				...
				Мы рассматриваем малую область с цилиндрической геометрией этой колонки зарядово-симметричной ($\mu=0$) равновесной нерелятивистской ($T\ll m$) плазмы электронов (электроны, ионы и др.) в относительно сильном поле $B\lesssim B_e$, направленном вдоль оси z. Электроны преимущественно будут занимать основной уровень Ландау.
				
				Внутри этой области $T=\text{const}$ и $B=\text{const}$. 
				
				В точке $z=0$ поддерживается стационарный поток фотонов, испытывающих только резонансное комптоновское рассеяние.
			\end{center}
		\end{column}
	\end{columns}
	% Справка
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{\small Введение}
	\centering
	\begin{itemize}
		\small
		\item[-] 	Используется 4-метрика с сигнатурой (+ -- -- --), 
		а также  система единиц $c = \hbar = k_B = 1$.
		\item[-] 	Элементарный заряд: $e>0$, масса электрона $m$.
		\item[-] 	Рассматривается система отсчета, где имеется только магнитное поле $\bf B$, направленное 
		вдоль третьей оси, а также используется калибровка Ландау $A^\mu=(0,0,xB,0)$
		\item[-]		
		Обезразмеренный тензор электромагнитного поля: 
		...
		\item[-] 4-векторы с индексами $\bot$ и $\parallel$ относятся 
		к подпространствам Евклида \{1, 2\} и Минковского \{0, 3\} соответственно.
		\item[-] 	Для произвольных векторов $p_\mu$, $q_\mu$ имеем:
		%
...
\begin{frame}{\small Поляризационные свойства фотонов}
	...
	\begin{center}	
		...
		($\mu=0$) векторы поляризации будут такими же, как и в чистом магнитном поле\\
			(T.A. Pukhov, D.A. Rumyantsev, M.V. Chistyakov  2026):
...
\begin{frame}{Параметры области излучения}
	%\scriptsize
	\centering
	Параметры аккреционных колонок \\ (P.A. Becker and M.T. Wolff 2022):
	\vspace{2mm}
	\begin{itemize}
		...
		\item[4.] Радиус от $4 \times 10^3$ см до $75 \times 10^3$ см (который также является радиусом горячего пятна на поверхности звезды).
		\item[5.] Высота источника излучения в зависимости от светимости варьируется как $h_s \sim 10^3 - 10^5$ см.
		\vspace{2mm}
		
		
		\item[6.] Размер области, в которых магнитное поле меняется не более чем на 5 \%, в дипольном приближении от $18 \times 10^3$ см до $34 \times 10^3$ см. (O. Nishimura 2011)
...
\begin{frame}{Методы решения задачи}
	...
	\begin{itemize}
		...
		P.A. Becker 2007, 2012, 2022 -- современная модель спектра с учетом движения плазмы без учета резонанса.  
...
\begin{frame}
	...
	\begin{itemize}		
		...
		R.A. Araya, A.K. Harding 1999, 2000 -- одни из первых, кто смоделировал циклотронные линии с учетом конечной ширины поглощения электрона методом Монте-Карло.
		\item[-]
...
\begin{frame}
	...
	Для первого уровня Ландау $n=1$ (Д.А. Румянцев и др. 2026):}
	\begin{equation}\nonumber
	\begin{aligned}
...
\begin{frame}{Анализ результатов}
	\small
	Томсоновская оптическая толща в поперечном и продольном направлении G. Sch\"onherr et. al.:
	\begin{equation}\nonumber
		\tau_{\shortparallel}^{(\text{\tiny T})} \simeq  \sigma_T n_e z,
	\end{equation}
	\begin{equation}\nonumber
		\tau_{\perp}^{(\text{\tiny T})} \simeq  \sigma_T n_e R,
	\end{equation}
	
	Условие $\tau_\parallel^{(\text{\tiny T})} >>\tau_\perp^{(\text{\tiny T})}$ приводит к тому, что фотоны в основном будут покидать колонку через её стенки.
	
	На масштабах $\tau_\perp^{(\text{\tiny T})} = 10^{-4} - 10^{-3}$ основной вклад дает резонансная область.
...
\begin{frame}{Анализ результатов}
	\begin{figure}\centering
		...
		\small
		Модель спектра распределения фотона, усредненного по объему колонки, поляризации фотона, а также по угловому диапазону $x=cos \theta=[0.5, 0.625)$ при магнитном поле $B=0.05 B_e$ и температуре $T=3$ кэВ, при концентрации $n_e=10^{19} \text{ см}^{-3}$ и $z_{\text{max}} \simeq 150$ м . Штриховая линия -- Sch\"onherr et. al. 2007.
	\end{figure}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{Анализ результатов}
	\begin{figure}\centering
		...
		\small
		Модель спектра распределения фотона, усредненного по объему колонки, поляризации фотона, а также по угловому диапазону $x=cos \theta=(0, 0.125)$ при магнитном поле $B=0.05 B_e$ и температуре $T=3$ кэВ, при концентрации $n_e=10^{19} \text{ см}^{-3}$ и $z_{\text{max}} \simeq 150$ м . Штриховая линия -- Sch\"onherr et. al. 2007.
	\end{figure}
\end{frame}
...
\begin{frame}{Что дальше?}
	...
	\begin{itemize}
		...
		\item[2)] Учет следующих поправок к пределу узкого резонансного пика (Румянцев Д.А., Ярков А.А., Шленев Д.М., 2026)
		\item[3)] Учет конечного радиуса колонки.
		\item[4)] Учет конечного изменения энергии $\Delta \omega$ в комптоновском процессе.
		\item[5)] Учет Доплеровского уширения, движения плазмы и т.д.
	\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
