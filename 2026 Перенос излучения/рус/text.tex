...
\title{Решение кинетического уравнения с учетом
...
Из закона сохранения энергии и $z$-компоненты импульса в реакции \linebreak$\gamma e \to \gamma e$  найдем:
\begin{equation}
\begin{aligned}
...
Далее, подставляя функции~(\ref{eq:Varphi}) в~(\ref{eq:SysEqALap}) и интегрируя получившийся результат по $x'$, с учетом $\Delta\omega\simeq \omega -\beta/m$ получим следующую бесконечную\linebreak систему зацепляющихся уравнений для нахождения коэффициентов $\overline{A}_\ell^{(\lambda)}(s,\omega)$:
...
\end{equation}
где $A$ -- нормировочный множитель, $\alpha$ -- спектральный индекс фотона, \linebreak $\omega_f$ -- энергия подавления. Функция распределения~(\ref{eq:functionZerof})  является простейшей моделью для описания спектра рентгеновских пульсаров, которая хорошо подходит для демонстрации влияния резонансного комптоновского рассеяния.
...
\begin{figure}\centering
	\includegraphics[width=0.9\linewidth,clip]{Fig6.eps}	\\
	\vspace{3mm}
