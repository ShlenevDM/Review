...
${\cal S}$-матричный элемент рассеяния фотона поляризации $\lambda$ на 
электроне в поляризационном состоянии $s$, находящемся на уровне Ландау $\ell$, 
с рождением электрона в поляризационном состоянии $s^{\, \prime}$ на уровне 
Ландау $\ell'$ и фотона поляризации $\lambda'$ с учетом 
лагранжиана~(\ref{eq:L}) может быть представлен в виде:  
%
\beq                          
\nonumber
...
Входящий в него пропагатор $S(Y, X)$ удобно рассматривать в виде разложения по уровням Ландау:
\begin{equation}
	S(Y, X) = \sum_{n=0}^{\infty}\sum_{s''=\pm 1} S_n^{s''} (Y, X)\, .
\end{equation}
...
\begin{eqnarray}
	\label{eq:propagator}
	S^{s''}_n (Y, X) && =  \int \frac{\dd p_0 \dd p_y \dd p_z}{(2\pi)^3} \times
	\\[3mm]
	\nonumber
	&& \times \frac{\eee^{- \, \ii \,  p_0 \,(Y_0 - X_0) + 
			\ii p_y \,(Y_2 - X_2) +  \ii \,  p_z \,(Y_3 - X_3)}}
	{p_0^2 - p_z^2 - M_n^2 + \ii\, {\cal I}^{s''}_\Sigma (p)} \times
	\\[3mm]
	\nonumber
	&& \times
	\, \phi^{s''}_{p, n} (Y_1) \bar \phi^{s''}_{p, n} (X_1) \, ,
\end{eqnarray}
%
...
\frac{|{\cal S}^{s^{\,\prime} s''}_{ e_n \to \gamma^{(\lambda')} 
		e_{\ell'}}|^2}{\tau}\, ,
\eeq 
где $f_{e}(E^{\, 	\prime}_{\ell'})=(1+\exp [E^{\, \prime}_{\ell'}/T])^{-1}$ -- равновесная функция распределения электронов с 
температурой $T$ и нулевым химическим потенциалом, $f_\gamma(\omega')=(\exp 
[\omega'/T]-1)^{-1}$ -- равновесная функция распределения фотонов, $\tau$ -- 
время взаимодействия.

Квадрат модуля\linebreak ${\cal S}$-матричного элемента, который определяет вероятность процесса и необходим при расчетах вычисляемых величин, таких как коэффициент поглощения фотона или сечение рассеяния фотона на электроне, может быть представлено в следующем виде:
%
\begin{eqnarray}
	\label{S_squared}
	...
	\\
	\nonumber
	&& = \sum\limits_{s, s', s'' = \pm 1} \, e^4
	\frac{(2\pi)^3 \delta^{(3)}_{0,y,z} (p+q - p^{\, \prime} - q^{\, \prime})}
	{2\omega L_x 2\omega^{\, \prime} V 2 E_\ell L_y L_z 2 E^{\, \prime}_{\ell^{\, \prime}} L_y L_z} \times
	\\
	\nonumber
	&&\times
	\left| \sum\limits_{n=0}^{\infty}
	\int \dd X_1 \dd Y_1 \ee_\mu^{(\lambda)} (q) \, \ee_{\mu'}^{*(\lambda')} (q') \eee^{\ii(Y_1 q'_x - X_1 q_x)}
	\right.
	\times
	\\
	\nonumber
	&&\times \frac{\phi_{p', \ell'}^{s'} (Y_1) \gamma_{\mu'} \phi^{s''}_{p+q, n} (Y_1) \bar \phi^{s''}_{p+q, n} (X_1)
		\gamma_{\mu} \phi_{p, \ell}^{s} (X_1)}
	{(p+q)^2_{\mprl}-M_n^2-\ii E_n''\Gamma^{s''}_n/2} +
	\\
	\nonumber
	&&+ (q, \lambda, \gamma_\mu \leftrightarrow -q', \lambda',  \gamma_{\mu'})\, \Big|^2 ,
\end{eqnarray}
%
где $E_n''=E_\ell+\omega$ -- энергия виртуального электрона и $(p+q)^2_{\mprl} = 
(E_\ell + \omega)^2 - (p_z + q_z)^2$.
...
другое состояние за счет тех или иных процессов, который можно представить в виде:
\begin{eqnarray}
	\label{eq:photon_abs}
	&&W_{\gamma^{(\lambda)} e_\ell \to \gamma^{(\lambda')} e_{\ell'}} = 
	\\[3mm]
	\nonumber
	&&=\sum\limits_{\ell, \ell' = 0}^{\infty} \; 
	\int  \frac{\dd p_y \dd p_z L_y L_z}{(2\pi)^2} \, f_e (E_{\ell}) \, 
	\dd W_{\gamma^{(\lambda)} e_\ell \to \gamma^{(\lambda')} e_{\ell'}} \, ,
\end{eqnarray}	
\begin{eqnarray}
	\label{dW}
	&&\dd W_{\gamma^{(\lambda)} e_\ell \to \gamma^{(\lambda')} e_{\ell'}} =  
...
Здесь $\xi= m z$, $x = \cos \theta$, $x' = \cos \theta'$ ($\theta$ и $\theta'$ -- углы между направлениями импульсов начального и конечного фотона и направлением магнитного поля соответственно),   $f^{(\lambda)}_\omega(\xi, x)$ и $f^{(\lambda')}_{\omega'}(\xi, x')$ -- подлежащие определению неравновесные функции распределения начального и конечного фотонов с энергиями $\omega$ и $ \omega'$ соответственно,  $f_E = 1/[\exp(E/T)+1]$ и $f_{E'} = 1/[\exp(E'/T)+1]$ --  равновесные функции распределения начального и конечного электронов с нулевым химическим потенциалом и энергиями начального и конечного электронов $E=\sqrt{p_z^2+M_\ell^2}$ и $E'=\sqrt{p_z^{\prime2}+M_{\ell \prime}^{ 2}}$ c соответствующими уровнями Ландау $\ell$ и $\ell'$, $dW_{\lambda \to \lambda'}$ -- дифференциальный коэффициент поглощения фотона~(\ref{dW}).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\beq
%\label{eq:wabs1} 
...
\begin{equation}\label{eq:KinSerDeltaw}
	...
	\end{aligned}
\end{equation}
где функции $\varphi^{\lambda \lambda'}_\omega(x,x')$ с учетом того, что в случае узкого резонансного пика амплитуда комптоновского процесса факторизуется амплитудами одновершинных процессов, могут быть представлены следующим образом:
\begin{equation}
...
\end{equation}
амплитуды одновершинных процессов могут быть представлены в виде~\cite{Rumyantsev:2017}:
%
\begin{eqnarray}
	\label{eq:amplonever} 
...
\end{eqnarray}
\begin{equation}
	{\cal M}^{s' s''}_{e_n \to \gamma^{(\lambda')} e_{\ell'}} = {\cal M}^{s'' s}_{\gamma^{(\lambda)} e_\ell \to e_n}(q \to q', E_\ell \to E_\ell', \lambda \to \lambda')
\end{equation}
